...
const makeMainHTML = () => {
	document.body.innerHTML = `<body style="background-color: #F2F2F2;">
      ...
      <section class="py-6" style="float: right; width: 22%; height: 702px;">
         <div class="container px-4 px-lg-5 mt-5">
            <div class="container1" >
               ...
               <div id="listContainer" style="height:369px;overflow: scroll;margin-bottom:10px;">
                  ${OrderListHTML()}
               </div>
         ...
         <div class="container px-4 px-lg-5 mt-5">
            ...
            <div class="col mb-6">
               <div class="card h-100">
                  <a id="Main" class="btn1 btn-outline-dark mt-auto" >메인 메뉴</a>
               </div>
            </div>
            <div class="col mb-6">
               <div class="card h-100">
                  <a id="Side" class="btn1 btn-outline-dark mt-auto" >사이드 메뉴</a>
               </div>
            </div>
            <div class="col mb-6">
               <div class="card h-100">
                  <a id="Drink" class="btn1 btn-outline-dark mt-auto">주류</a>
               </div>
            </div>
            <div class="col mb-6">
               <div class="card h-100">
                  <a id="Beverage" class="btn1 btn-outline-dark mt-auto" >음료</a>
               </div>
            </div>
            <div class="col mb-6">
               <div class="card h-100">
                  <a id="Receipt" class="btn1 btn-outline-dark mt-auto">주문 내역</a>
               </div>
            </div>
            <div class="col mb-7">
               <div class="card h-100">
		...
		<div class="modal-dialog">
			<div class="modal-content" >
				...
				<!-- Modal body -->
					...
					나눠 낼 인원 : <span id="dutchMinus" class="material-symbols-outlined"
						onclick='count("minus")'> indeterminate_check_box </span> <span
						id='result'>1</span> <span id="dutchPlus" class="material-symbols-outlined"
						onclick='count("plus")'> add_box </span>
				</div>

				<!-- Modal footer -->
				<div class="modal-footer bar1"></div>
...
const getReceiptList = () => {
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		success: (receiptList) => jsonReceiptList = receiptList,
		error: (e) => alert(e)
	});
}
setInterval(()=>getReceiptList(), 1000);

const ReceiptListHTML = () => {
	totalPrice = 0;
	for (let i = 0; i < jsonReceiptList.length; i++) {
		receiptListText += `<tr>
											<td style="position: relative; right: 95px; top: 20px">${jsonReceiptList[i].menu_name}</td>
											<td class="right"
												style="position: relative; top: 20px; right: 10px"><span
												class="price">${jsonReceiptList[i].menu_price}</span></td>
											<td style="position: relative; left: 74px; top: 20px">${jsonReceiptList[i].order_cnt}</td>
											<td style="position: relative; left: 110px; top: 20px"><span
												class="price">${jsonReceiptList[i].order_cnt * jsonReceiptList[i].menu_price}</span></td>
										</tr>`

		totalPrice += parseInt(jsonReceiptList[i].order_cnt) * parseInt(jsonReceiptList[i].menu_price)
	}

	return receiptListText;
}
			...
			<div style="width: 770px">
				...
				<section class="py-6">
					...
					<div class="container">
						...
						<div class="receipt">
							<div class="paper">
								<div class="date" style="font-size: 15px;">${jsonReceiptList[0].receipt_date}</div>
								<div class="title">
								...
								<footer style="margin-top: 20px;">
									<dl class="info">
										<div class="total">
											...
											<h2>
												총액 &nbsp; :&nbsp; <span class="price">${totalPrice}원</span>
										</div>
									</dl>
									<tr>
...
const Ordering = () => {
	...
	h_speech.onresult = function(e) {
		...
		if (h_text.indexOf("영수증") !== -1 || h_text.indexOf("내역") !== -1) {
			receiptListText = "";
		totalPrice=0;
		getReceiptList();
		setTimeout(() => {
...
delete
	...
	document.getElementById("ordertrue").addEventListener("click", orderT);
...
const dutchEvent = () => {
	document.getElementById("dutchPlus").addEventListener("click", count('plus'))
	document.getElementById("dutchMinus").addEventListener("click", count('minus'))
}
